...
class BuildTarget:
    ...
    def depsAreVirtual(self) -> bool:
        ...
        for d in self.producedby.depends:
            if (
                d.producedby.rulename.name == "phony"
    ...
    def visit_graph(
        ...
        if self.is_a_file or not (
            self.producedby.rulename.name == "phony"
    ...
    def print_graph(self, ident: int = 0):
    ...
    def _handleCmdForBazelGen(self, cmd: str, el: "BuildTarget", ctx: Dict[str, Any]):
        ...
        if ("c++" in cmd or "g++" in cmd) and "-c" in cmd:
            ctx["dest"] = ctx["bazelbuild"].bazelTargets[-1]
        ...
        if "/ar " in cmd:
            ...
            ctx["bazelbuild"].bazelTargets[-1].addDep(t)
            ctx["bazelbuild"].bazelTargets.append(t)
            return
        logging.debug(cmd)

    def gen_bazel(self, bb: BazelBuild):
        def visitor(el: "BuildTarget", ctx: Dict[str, Any]):
            ctx["bazelbuild"] = bb
            if el.producedby:
                ...
                ctx["dest"].addSrc(el.name)

        def setup(ctx):
            ctx2 = {k: v for k, v in ctx.items()}
            return ctx2

        ctx = {"setup_subcontext": setup}
...
class Build:
    def __init__(
        ...
        rulename: Rule,
        inputs: Optional[List[BuildTarget]],
        depends: Optional[List[BuildTarget]],
    ):
    ...
    def _handleBuild(self, arr: List[str]):
        ...
        for i in raw_inputs:
            if not i.startswith("/"):
                ...
                p = i
            if os.path.exists(p):
                inputs.append(BuildTarget(i).markAsFile())
            else:
                v = self.all_outputs.get(i)
                if not v:
                    v = BuildTarget(i)
                    v.markAsUnknown()
                    self.missing[i] = v
...
def get_top_levels(parser: NinjaParser) -> List[BuildTarget]:
    real_top_targets = []
...
def main():
    ...
    cur_dir = os.path.dirname(os.path.abspath(filename))
    return handleBuildFile(raw_ninja, cur_dir)


def handleBuildFile(raw_ninja: List[str], dir: str):
    parser = NinjaParser()
    ...
    top_levels = get_top_levels(parser)
    bb = BazelBuild()
    for e in top_levels:
        e.print_graph()
        e.gen_bazel(bb)

    logging.debug("\n  " + "\n  ".join([str(b) for b in bb.bazelTargets]))
